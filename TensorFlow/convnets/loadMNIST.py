...
def load_data(flatten=True):
	xTrain = load_images('train-images-idx3-ubyte.gz','data/',flatten)
	tTrain = load_labels('train-labels-idx1-ubyte.gz','data/')
	xTest = load_images('t10k-images-idx3-ubyte.gz','data/',flatten)
...
def load_images(filename, path='', flatten=True):
	maybe_download(filename, path)
	with gzip.open(path + filename, 'rb') as f:
		data = np.frombuffer(f.read(), np.uint8, offset=16)

	data = data.reshape(-1, 28 * 28) / np.float32(256)
	if flatten:
		return data
	else:
		sh = data.shape
		return data.reshape([sh[0],28,28, 1])
...
def get_MNIST(flatten=True):
	xTrain, xValid, xTest, tTrain, tValid, tTest = load_data(flatten)
